...
import { PlayerCashUpdatedEvent } from "Manager_Events";
const UPDATE_MS = 500; // pull fresh stats twice per second
const ANIM_STEP_MS = 50; // ~20 fps for smooth counting

export default class HUD_PlayerCashTop extends UIComponent<typeof HUD_PlayerCashTop> {
  ...
  private playerCashBgAssetId = "897510732618022";
  private fontSizeBinding = new Binding<number>(24);
  ...
  start() {
    // this.refreshTargets();
    // this.animateCoins();
    this.connectNetworkEvent(this.entity.owner.get(), PlayerCashUpdatedEvent, (event) => {
      this.refreshTargets(event.newCash);
    });
  ...
  private refreshTargets(real: number): void {
    ...
    if (!this.displayedCoins.has(player)) {
      ...
      this.coinsBinding.set(this.formatCoin(real));
    ...
    if (prevTarget !== undefined) {
      ...
      if (delta > 0) {
        ...
        this.flyerCoinBinding.set(flyerText);
